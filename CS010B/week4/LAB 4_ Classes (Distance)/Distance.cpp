...
unsigned Distance::getFeet() const{
   ...
   */
   Distance Distance::operator-(const Distance &rhs) const{
    int totalFeet = static_cast<int>(this->getFeet()) - static_cast<int>(rhs.getFeet());
    double totalInches = this->getInches() - rhs.getInches();
    ...
    if (totalFeet < 0) {
        totalFeet = abs(totalFeet);
        totalInches = abs(totalInches);
    }

    return Distance(static_cast<unsigned>(totalFeet), totalInches);
}
